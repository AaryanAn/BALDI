...
export class DrawingSession {
  ...
  /** Finished + current strokes; only `clear()` resets this. */
  private paths: Trajectory[] = []
  private currentStroke: Trajectory | null = null
  ...
  /** All strokes for drawing (each segment separately). */
  allPaths(): Trajectory[] {
    return this.paths
  }
  ...
  private updatePath(point: Point, pinchActive: boolean): void {
    ...
    if (this.drawing && pinchActive && this.currentStroke) {
